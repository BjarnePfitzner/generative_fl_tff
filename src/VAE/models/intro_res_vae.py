...
CHANNELS = {
    28: [16, 32],
    128: [16, 32, 64, 64, 64],
    256: [16, 32, 64, 128, 128, 128],
    #256: [16, 32, 64, 128, 256, 512, 512, 512],
    #512: [16, 32, 64, 128, 256, 512, 512, 512],
    1024: [16, 32, 64, 128, 256, 512, 512, 512]
}


class IntroResVAE(BaseVAE):
    def __init__(self, latent_dim, data_dim, data_ch, n_classes, output_activation_fn):
        ...
        super(IntroResVAE, self).__init__(latent_dim, data_dim, data_ch, n_classes, output_activation_fn)
    ...
    def define_decoder(self, latent_input, label_input):
        cc = self.channels[-1]
        initial_dim = int(self.data_dim / (2**len(self.channels)))
        x = layers.Dense(cc*initial_dim*initial_dim, activation='relu', name='decoder/dense_1')(latent_input)
        x = layers.Reshape((initial_dim, initial_dim, cc), name='decoder/Reshape')(x)

        l = layers.Embedding(self.n_classes, 10, name='encoder/embedding')(label_input)
        l = layers.Dense(initial_dim * initial_dim, name='decoder/dense')(l)
        l = layers.Reshape((initial_dim, initial_dim, 1), name='decoder/reshape_embedding')(l)
...
def define_vae(latent_dim, data_dim, data_ch, n_classes, output_activation_fn='linear'):
    return IntroResVAE(latent_dim=latent_dim,
                       data_dim=data_dim,
                       data_ch=data_ch,
                       n_classes=n_classes,
